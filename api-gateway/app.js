...
const orderServiceIp = process.argv[2];
const paymentServiceIp = process.argv[3];
                    ...
                    res2.on('data', function(data2) {
                        ...
                        orders.forEach((order) => {
                            transactions.forEach((transaction) => {
                                if(orders.id == transaction.orderId) {
                                    result.push({id: order.id, price: order.price, customer: order.customer, success: transaction.success})
                                }
                            })
                        })
                        res.json(result);
